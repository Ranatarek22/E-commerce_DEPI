  ...
  return getCartAmount() !== 0 ? (
    <div className='border-t pt-14'>
          ...
          cartData.map((item, index)=>{
            ...
            return(
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                  ...
                  <input onChange={(e)=>e.target.value === '' || e.target.value === '0' ? 'null' : updateQuantity(item._id, item.size, Number(e.target.value))} className='border w-20 px-1 py-1' type='number' min={1} value={item.quantity}/>
      ...
      <div className='flex mt-20 mb-20 gap-28'>
        <CartTotal/>   
        <div>
          <button onClick={()=>navigate('/placeOrder')} className='mt-16 mb-5 bg-black text-white px-8 py-3 hover:bg-gray-800 text-sm'>PROCEED TO CHECKOUT</button>  
        </div>
      </div>
    </div>
  ): <div className='flex flex-col items-center gap-9 border-t'>
    <h1 className='font-light text-2xl mt-20'>YOUR CART IS EMPTY</h1>
    <img className='w-80' src={assets.emptyCart} alt=''/>
  </div>
}
